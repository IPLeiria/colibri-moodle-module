...
if(!defined('COLIBRI_PLUGINNAME')):
    define('COLIBRI_PLUGINNAME', 'colibri');
endif;
...
if(!class_exists('Colibri')):
	...
	public static function getSessionUsers($resourceId){
	    if($users = get_records('colibri_users', 'colibrisessionid', $resourceId)):
		return $users;
	    endif;
	    return null;
	}
	...
	public static function hasUserSessionAdminPriviligies($resourceId, $userId){
	    return record_exists('colibri', 'id', $resourceId, 'creatorid', $userId);
	}
	...
	public static function hasUserAccessToSession($resourceId, $userId){
	    if(self::hasUserSessionAdminPriviligies($resourceId, $userId) || record_exists('colibri_users', 'colibrisessionid', $resourceId, 'userid', $userId) || count_records('colibri_users', 'colibrisessionid', $resourceId)<get_field('colibri', 'maxsessionusers', 'id',$resourceId)):
		return true;
	    endif;
	    return false;
	}
	...
	public static function reserveSeatForUserInSession($resourceId, $userId){
		...
		if(record_exists('colibri_users', 'colibrisessionid', $resourceId, 'userid', $userId)):
		    return true;
		endif;
		$dbUser = new stdClass();
		...
		return insert_record('colibri_users', $dbUser, false);
	    endif;
	    return false;
	}
	...
	public static function createSession($userId, $courseId, sessionScheduleInfo $sessionInfo, $users=array(), $intro='', $introformat=0){
	    ...
	    if($resource->id = insert_record('colibri', $resource)):
		if(!empty ($users)):
			...
			insert_record('colibri_users', $dbUser);
	...
	public static function modifySession($userId, $instanceId, sessionScheduleInfo $sessionInfo, $users=array(), $intro='', $introformat=0){
		...
		if(update_record('colibri', $resource)):
		    if(!empty ($users)):
			// remove the old records
			delete_records('colibri_users', array('colibrisessionid'=>$resource->id));
			foreach ($users as $user):
			    $dbUser = new stdClass();
			    $dbUser->colibrisessionid = $resource->id;
			    $dbUser->userid = $user;
	...
	public static function removeSession($userId, $instanceId){
		...
		if(has_capability('mod/colibri:managesession', $coursecontext, $userId) && ColibriService::removeSession($dbSession->sessionuniqueid)===true):
		    return (delete_records('colibri_users', array('colibrisessionid'=>$instanceId)) && delete_records('colibri', array('id'=>$instanceId)));
		endif;
	    endif;
	    return false;
	}
	...
	public static function getSessionInfo($resourceId, $live=false){
		...
		$updateMethod = $CFG->colibri_session_information_update_method;
	    endif;

	    switch($updateMethod):
		case ColibriService::LOCAL_INFORMATION_METHOD:
		case ColibriService::CRON_INFORMATION_METHOD:
		    if ($session = get_record('colibri', 'id', $resourceId)):
			$session->users = get_records('colibri_users', 'colibrisessionid', $session->id ,'', 'userid, type, accessed');
	...
	public static function syncSessionsInformation(){
		...
		if($sessionsTmp = get_records('colibri')):
